...
/* if ($_GET['filter'] == 'saved') {
    $treks_filtered = lxp_get_teacher_saved_treks($student_post->ID, $treks_saved, urldecode($_GET['strand']), urldecode($_GET['sort']), urldecode($_GET['search']));
} else if ($_GET['filter'] == 'recent'){
    $lxp_visited_treks = get_post_meta($student_post->ID, 'lxp_visited_treks');
